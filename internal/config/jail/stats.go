...
func racctToCPUStats(entries map[string]uint64) *cgmgr.CPUStats {
	cpu := &cgmgr.CPUStats{}
	if val, ok := entries["cputime"]; ok {
		...
		cpu.TotalUsageNano = val*1000000000 + 1
	}
	return cpu
}

func racctToMemoryStats(entries map[string]uint64) *cgmgr.MemoryStats {
	memory := &cgmgr.MemoryStats{}
	if val, ok := entries["memoryuse"]; ok {
		memory.WorkingSetBytes = val
		memory.RssBytes = val
	}
...
func PopulateContainerStats(jailName string, stats *cgmgr.CgroupStats) error {
	entries, err := getRacct("jail:" + jailName)
	if err != nil {
		return fmt.Errorf("unable to read accounting for %s: %w", jailName, err)
	}

	stats.CPU = racctToCPUStats(entries)
	stats.Memory = racctToMemoryStats(entries)
	return nil
}
...
func PopulateSandboxStats(jailName string, sandboxStats *types.PodSandboxStats) error {
	...
	cpu := racctToCPUStats(entries)
	memory := racctToMemoryStats(entries)

	now := time.Now().UnixNano()
	stats.Cpu = &types.CpuUsage{
		Timestamp:            now,
		UsageCoreNanoSeconds: &types.UInt64Value{Value: cpu.TotalUsageNano},
	}
	stats.Memory = &types.MemoryUsage{
		Timestamp:       now,
		WorkingSetBytes: &types.UInt64Value{Value: memory.WorkingSetBytes},
		RssBytes:        &types.UInt64Value{Value: memory.RssBytes},
		PageFaults:      &types.UInt64Value{},
		MajorPageFaults: &types.UInt64Value{},
		UsageBytes:      &types.UInt64Value{Value: memory.MaxUsage},
